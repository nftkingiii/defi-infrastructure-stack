...
contract PerpRiskParams {
    ...
    address public immutable perpDexSetter;
    ...
    constructor(address _scoreRegistry, address _perpDex) {
        ...
        perpDex = _perpDex;
        perpDexSetter = msg.sender;
    }

    function setPerpDex(address _perpDex) external {
        if (msg.sender != perpDexSetter) revert Unauthorised(msg.sender);
        if (perpDex != address(0) || _perpDex == address(0)) revert ZeroAddress();
        perpDex = _perpDex;
    ...
    function syncCircuitBreaker(bytes32 poolId) external returns (bool halted) {
        ...
        (uint8 riskScore,,,, uint8 confidence, uint48 scoreTimestamp) = scoreRegistry.getRiskData(poolId);
        uint48 scoreAge = uint48(block.timestamp) - scoreTimestamp;

        if (scoreAge > MAX_SCORE_AGE_SECONDS) {
            _tripCircuitBreaker(poolId, "Score stale");
        } else if (confidence < MIN_CONFIDENCE) {
            _tripCircuitBreaker(poolId, "Confidence too low");
        } else if (riskScore < MIN_RISK_SCORE_TO_TRADE) {
            _tripCircuitBreaker(poolId, "Risk score critical");
        }

        return _circuitBreakers[poolId].halted;
    }
    ...
    function _deriveParams(
        ...
        p.maxLeverage =
            rawLeverage > MAX_LEVERAGE ? MAX_LEVERAGE : rawLeverage < MIN_LEVERAGE ? MIN_LEVERAGE : rawLeverage;
        ...
        p.riskScoreUsed = riskScore;
    }

    function _haltedParams() internal view returns (RiskParams memory) {
        return RiskParams({
            maxOI: 0,
            maxLeverage: 0,
