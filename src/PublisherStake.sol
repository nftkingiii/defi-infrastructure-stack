...
event PublisherSlashed(
    address indexed publisher, address indexed watchdog, uint128 shMonSlashed, uint128 watchdogBounty, uint128 burned
);
...
error ZeroAmount();
error UnauthorisedSetter(address caller);

contract PublisherStake {
    uint48 public constant UNBONDING_PERIOD = 38 days;
    ...
    address public immutable adjudicatorSetter;
    ...
    constructor(address _shMon, address _adjudicator, uint128 _minStakeMon) {
        if (_shMon == address(0)) revert ZeroAddress();
        shMon = IShMON(_shMon);
        adjudicator = _adjudicator;
        adjudicatorSetter = msg.sender;
        minStakeMon = _minStakeMon;
        watchdogBountyBps = DEFAULT_BOUNTY_BPS;
    }

    function setAdjudicator(address _adjudicator) external {
        if (msg.sender != adjudicatorSetter) revert UnauthorisedSetter(msg.sender);
        require(adjudicator == address(0), "Adjudicator already set");
        if (_adjudicator == address(0)) revert ZeroAddress();
        adjudicator = _adjudicator;
    }
