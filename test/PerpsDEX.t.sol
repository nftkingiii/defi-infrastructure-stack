...
contract PerpsDEXTest is Test {
    ...
    function setUp() public {
        ...
        adjudicator = new DeviationAdjudicator(address(publisherStake), address(scoreRegistry), address(shMon), owner);
        perpRiskParams = new PerpRiskParams(address(scoreRegistry), address(0));
        perpsDex = new PerpsDEX(address(perpRiskParams), address(usdc));
        perpRiskParams.setPerpDex(address(perpsDex));
...
contract PerpsDEXPositionTest is PerpsDEXTest {
    function test_OpenLongPosition() public {
        ...
        assertEq(pos.riskScoreAtOpen, 80);
    ...
    function test_PnlUsesEntryPriceAsDenominator() public {
        vm.prank(owner);
        perpsDex.updatePrice(POOL_ID, 20_000);
        ...
        vm.prank(owner);
        perpsDex.updatePrice(POOL_ID, 22_000);
        vm.prank(trader);
        perpsDex.closePosition(posId);

        assertEq(usdc.balanceOf(trader), balStart + 200e6);
    }
...
contract PerpsDEXLiquidationTest is PerpsDEXTest {
    ...
    function test_LiquidationTrackedByScoreBucket() public {
        ...
        vm.prank(liquidator);
        perpsDex.liquidate(posId);
        (uint256 liquidations,,) = perpsDex.getLiquidationRate(8);
    ...
    function test_LiquidationRateComputed() public {
        ...
        (uint256 liq, uint256 pos, uint256 rateBps) = perpsDex.getLiquidationRate(8);
        assertEq(liq, 1);
        assertEq(pos, 2);
...
contract PerpsDEXFundingTest is PerpsDEXTest {
    function test_FundingIndexIsTracked() public {
        ...
        assertGt(fundingAfter, fundingAtOpen);
        ...
        assertEq(balAfter, balBefore + 979_800_000);
    }
}
